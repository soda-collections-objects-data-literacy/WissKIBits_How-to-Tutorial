...
### Task 2: Initial mapping to CIDOC CRM

Look at your model sketch again and select **two terms** from it, e.g. game, person, organization, title, or genre.

For each term, look for a **CIDOC CRM class** that could correspond to the meaning of the term.

Briefly justify your choice of class.

**Note:**

> The aim is not yet to create a complete or final CIDOC CRM model.
> The key question at this stage is: What do we mean by our term – and which class describes this meaning as appropriately as possible?

**Mini-demo: CIDOC CRM as a toolkit** 

For an initial orientation, the following classes may be helpful:
...
**From a name to an Appellation**

In our initial model sketch, we can first formulate simply:

> Game → has an appellation → “The Legend of Zelda: A Link to the Past”

CIDOC CRM makes it possible to model such a designation more precisely. E41 Appellation denotes an appellation by which an instance of a CRM class can be identified or referred to.

For titles, E35 Title provides a more specific class: E35 Title is a subclass of E41 Appellation. A title is therefore a particular form of an appellation.

In simplified form, we can distinguish:

> E41 Appellation
> → general appellation
>
> E35 Title
> → particular form of an appellation: a title
>
> E42 Identifier
> → particular form of an appellation: an identifier

This makes clear that terms such as appellation, title, and identifier are not the same in CIDOC CRM, but they are part of a shared conceptual relationship.

**Key takeaway**

> Before selecting a class, check whether the Scope Note of the CIDOC CRM class corresponds to the meaning of the concept term in the domain model.

The precise modeling of appellations, their symbolic content, and datatype properties is covered in Module 3.

---

### Task 2: Discussion of results in the plenary session

***Sample example: From the domain model to CIDOC CRM modeling***

The model sketch created in the exercise initially describes the concepts, events, and relationships of the example domain. In the next step, these elements can be further formalized using classes (Entities) and properties (Properties) of CIDOC CRM.

The following figure provides an example of how such a model sketch can be developed into a more formalized semantic model:

![Concept mind map](../WissKIBits_Modul1/assets/Mindmap.png)

> **Figure:** The figure shows a sample mind map for the computer game “The Legend of Zelda: A Link to the Past”.

The initially freely formulated elements and relationships are gradually transformed into CIDOC CRM classes and properties. The figure should therefore not be understood as the only possible solution, but as a modeling proposal that can be reviewed and developed further.

**Note:** 

> Semantic modeling does not only mean finding suitable classes.
> Modeling decisions make explicit what meaning we assign to a term and which relationships our data are intended to express.
